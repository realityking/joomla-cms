...
/**
 * Configuration model for the Joomla Core Installer.
 *
 * @package     Joomla.Installation
 * @subpackage  Model
 * @since       3.0
...
class InstallationModelConfiguration extends JModelBase
{
	...
	public function setup($options)
	{
		...
		{
			// Message was set within the method
			return false;
		}
	...
	private function _createRootUser($options)
	{
		...
		else
		{
			$query = $db->getQuery(true);
			$columns = array(
				$db->quoteName('id'), $db->quoteName('name'), $db->quoteName('username'), $db->quoteName('email'), $db->quoteName('password'),
				$db->quoteName('usertype'), $db->quoteName('block'), $db->quoteName('sendEmail'), $db->quoteName('registerDate'),
				$db->quoteName('lastvisitDate'), $db->quoteName('activation'), $db->quoteName('params')
			);
			$query->insert($db->quoteName('#__users'), true);
			$query->columns($columns);

			$query->values($db->quote($userId) . ', '
				. $db->quote('Super User') . ', '
